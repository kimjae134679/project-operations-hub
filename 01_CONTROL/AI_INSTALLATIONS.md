...
| jev-router | 0.2.0 | `C:\Users\user\AppData\Roaming\npm` | npm 전역 기본 위치 유지. API 키 설정 및 Threads 저장소에서 `jev-codex` 실제 라우팅 확인 |
| AI Ops Runner | 준비 중 | `project-operations-hub/ai-ops-runner` | GitHub Issue → 로컬 `jev-codex` → 브랜치/PR 흐름 구현. PC 작업 스케줄러 설치와 실제 왕복 검증은 아직 미실행 |
